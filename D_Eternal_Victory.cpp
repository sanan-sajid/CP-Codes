...
// int res = 0;
// void dfs(int node, int parent, vector<vector<vector<int>>> &adj)
// {
//   int sum = 0;
//   int maxi = 0;
...
//     {
//       sum += adj[node][i][0];
//       maxi = max(maxi, adj[node][i][0]);
//     }
//   }
//   debug(node, (2 * (sum - maxi) + maxi), sum, maxi);
//   if (!(adj[node].size() == 1 && adj[node][0][1] == parent))
//     res += (2 * (sum - maxi) + maxi);
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    while (!st.empty())
    {
      ...
      for (auto it : adj[node])
      {
        if (dis[it[1]] > it[0] + dis[node])
        {
          dis[it[1]] = it[0] + dis[node];
          st.insert({dis[it[1]], it[1]});
        }
      }
    }
    cout << sum - *max_element(all(dis));
    cout
        << '\n';
  }
  return 0;
