...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    vector<pair<char, int>> idk(26);
    vector<pair<char, int>> idk1(26);
    int noIdea;
    for (int i = 0; i < 26; i++)
    {
      v[i].first = i + 'a';
    ...
    sort(all(v), sortd);
    idk = v;
    // i -> unique char in string
    int res = LONG_LONG_MAX;
    for (int i = 1; i <= 26; i++)
    {
      if (n % i != 0)
        continue;
      int charPerAlphabet = n / i;
      int currRes = 0;
      int h = i - 1, l = 0;
      int k = i;
      int temp = 0;
      ...
      while (h > l)
      {
        int req = n / i - v[h].second; // 1
        diff += req;
        v[h].second = n / i;
        while (req)
        {
          req -= min(v[l].second, req);
          v[l].second = max(0LL, v[l].second - n / i);
          l++;
        }
        h--;
      }
