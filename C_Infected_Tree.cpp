...
int dfs(int node, int parent, vector<vector<int>> &adjList)
{
  ...
  {
    if (it != parent)
      ans += dfs(it, node, adjList);
  }

  return dp[node] = ans;
}

int rec(int node, int parent, vector<vector<int>> &adjList)
{

  // handle root node seprately
  if (parent == -1)
  {
    int l = 0, r = 0;
    if (adjList[node].size() == 1)
    {
      l = dfs(adjList[node][0], node, adjList);
      return l - 1;
    }
    if (adjList[node].size() == 2)
    {
      l = dfs(adjList[node][0], node, adjList);
      r = dfs(adjList[node][1], node, adjList);
      return max((l - 1) + rec(adjList[node][1], node, adjList), (r - 1) + rec(adjList[node][0], node, adjList));
    }
  }

  int l = 0, r = 0;
  if (adjList[node].size() == 3)
  {
    if (adjList[node][2] == parent)
    {
      l = dfs(adjList[node][0], node, adjList);
      r = dfs(adjList[node][1], node, adjList);
      return max((l - 1) + rec(adjList[node][1], node, adjList), (r - 1) + rec(adjList[node][0], node, adjList));
    }
    else if (adjList[node][1] == parent)
    {
      l = dfs(adjList[node][2], node, adjList);
      r = dfs(adjList[node][0], node, adjList);
      return max((l - 1) + rec(adjList[node][0], node, adjList), (r - 1) + rec(adjList[node][2], node, adjList));
    }
    else
    {
      l = dfs(adjList[node][2], node, adjList);
      r = dfs(adjList[node][1], node, adjList);
      return max((l - 1) + rec(adjList[node][1], node, adjList), (r - 1) + rec(adjList[node][2], node, adjList));
    }
  }
  else if (adjList[node].size() == 2)
  {
    int l;
    if (adjList[node][0] != parent)
      l = dfs(adjList[node][0], node, adjList);
    else
      l = dfs(adjList[node][1], node, adjList);
    debug(l);
    return l - 1;
  }
  else
  {
    ...
    if (adjList[node][0] != parent)
    {
      return dfs(adjList[node][0], node, adjList) - 1;
    }
    return 0;
  }
}
