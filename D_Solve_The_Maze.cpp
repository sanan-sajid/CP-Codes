  ...
  void f(int i, int j)
  {
    for (int a = 0; a < 4; a++)
    {
      if (valid(i + dx[a], j + dy[a]) && v[i + dx[a]][j + dy[a]] == '.')
      {
        v[i + dx[a]][j + dy[a]] = '#';
      }
    }
  }
  bool dfs(int i, int j, vector<vector<bool>> &vis, vector<vector<int>> &dp)
  {
    ...
    for (int a = 0; a < 4; a++)
    {
      ans = (ans | dfs(i + dx[a], j + dy[a], vis, dp));
    }
    return dp[i][j] = ans;
  }
  int32_t main()
