...
int f(int i, int mex, vector<int> &v, vector<vector<int>> &dp)
{
  if (i == 0)
  {
    return mex * (v[0] - 1);
  ...
  if (dp[i][mex] != -1)
    return dp[i][mex];
  return dp[i][mex] = min(f(i - 1, mex, v, dp), f(i - 1, i, v, dp) + mex * (v[i] - 1) + i);
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    {
      vector<vector<int>> dp(mex, vector<int>(mex + 1, -1));
      debug(cnt, mex);
      cout << f(mex - 1, mex, cnt, dp);
    }
    cout << '\n';
  }
  return 0;
