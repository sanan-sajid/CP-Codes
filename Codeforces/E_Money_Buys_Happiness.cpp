...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (auto it : v)
      sm += it.second;
    // dp[i][j]->min cost for j happy and at ith index
    vector<vector<int>> dp(m, vector<int>(sm + 1, 1e18));
    dp[0][0] = 0;
    if (v[0].first == 0)
    {
      dp[0][v[0].second] = 0; // else not poss as 0 money hoga
    }
    for (int i = 1; i < m; i++)
    {
      for (int j = 0; j <= sm; j++)
      {
        ...
        if (j - v[i].second >= 0 && (i)*x >= dp[i - 1][j - v[i].second] + v[i].first)
