...
vector<vector<int>> dp(4 * 1e4 + 5, vector<int>(500, -1));
const int modo = 1e9 + 7;
int helper(vector<int> &paliNums, int currSum, int index)
{
  if (currSum < 0)
    return 0;
  if (currSum == 0)
    return 1;
  if (index >= paliNums.size())
    return 0;
  if (dp[currSum][index] != -1)
    return (dp[currSum][index] % modo);
  int notTaking = (helper(paliNums, currSum, index + 1) % modo);
  int taking = (helper(paliNums, currSum - paliNums[index], index) % modo);
  return dp[currSum][index] = (notTaking + taking) % modo;
}
int32_t main()
{
  ...
  ll t = 1;
  cin >> t;
  vector<int> paliNumbs;

  for (int i = 1; i <= 4 * 1e4; i++)
  {
    if (checkPalindrome(i))
      paliNumbs.push_back(i);
  }
  while (t--)
  {
    int n;
    cin >> n;
    // for (int i = 0; i <= n; i++)
    //   cout << paliNumbs[i] << endl;
    cout << helper(paliNumbs, n, 0);
