...
int countConsecutiveElements(const std::vector<int> &arr)
{
    if (arr.empty())
    {
        return 0;
    ...
    return std::max(maxConsecutiveCount, consecutiveCount);
}
int32_t main()
{
    ...
    while (t--)
    {
        ...
        else
        {
            ...
            {
                int res = (v.size() / 2) * y;
                // cout << (v.size() / 2) * min(x, y);
                int numOfCons = countConsecutiveElements(v) / 2;
                // cout << numOfCons << "yo\n";
                res = min(res, (numOfCons * x + ((v.size() - numOfCons * 2) / 2) * y));
                int idk = 0;
            ...
            else
            {
                ...
                if (num2 - num1 > 1)
                {
                    cout << min({y, x * (num2 - num1)});
                }
                else
                {
                    int res = x;
                    if (num1 > 1 || n - 1 - num2 > 1)
                    {
                        res = min(res, 2 * y);
                    }
