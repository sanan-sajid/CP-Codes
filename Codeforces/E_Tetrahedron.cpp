...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    int MOD = 1e9 + 7;
    cin >> n;
    // vector<vector<int>> dp(1e5 + 5, vector<int>(4));
    // int N = 1e5 + 5;
    int dp[2][4];
    ...
    for (int j = 1; j <= n; j++)
    {
      int i = j % 2;
      int previous = 1 - i;
      dp[i][0] = (dp[1 - i][1] % MOD + dp[1 - i][2] % MOD + dp[1 - i][3] % MOD) % MOD;
      dp[i][1] = (dp[1 - i][0] % MOD + dp[1 - i][2] % MOD + dp[1 - i][3] % MOD) % MOD;
      dp[i][2] = (dp[1 - i][1] % MOD + dp[1 - i][0] % MOD + dp[1 - i][3] % MOD) % MOD;
      dp[i][3] = (dp[1 - i][1] % MOD + dp[1 - i][2] % MOD + dp[1 - i][0] % MOD) % MOD;
    }
    cout << dp[n % 2][3] % MOD;
