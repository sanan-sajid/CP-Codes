...
int dfs(vector<vector<int>> &adjList, int node, int parent, vector<int> &subtreeSize)
{
  ...
  for (auto it : adjList[node])
  {
    if (parent != it)
    {
      ans += (1 + (dfs(adjList, it, node, subtreeSize)));
    }
  }
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    // level[1] = 0;
    stack<pair<int, pair<int, int>>> st; // parent,node,height
    st.push({-1, {1, 0}});
    while (!st.empty())
    {
      int parent = st.top().first;
      int node = st.top().second.first;
      int height = st.top().second.second;
      level[node] = height;
      st.pop();
      for (auto it : adjList[node])
      {
        if (parent != it)
        {
          st.push({node, {it, height + 1}});
        }
      }
    }
    vector<int> subtreeSize(n + 1);
    dfs(adjList, 1, -1, subtreeSize);
    // cout << level << endl;
    // cout << subtreeSize;
    multiset<int> ms;
    for (int i = 1; i <= n; i++)
    {
      ms.insert(level[i] - subtreeSize[i]);
    }
    debug(ms);
    k = n - k;
    // k--;
