...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = j; i >= 0; i--)
    {
      ...
      int temp = 0;
      // temp += pp.order_of_key(v[i]);
      temp += pp1.order_of_key(v[j] + 1) - pp1.order_of_key(v[i] - 1);
      temp += pp.order_of_key(2 * v[i]);
      maxi = max(maxi, temp);
      debug(pp1.order_of_key(v[j] + 1) - pp1.order_of_key(v[i]), pp.order_of_key(2 * v[i]));
      jj++;
    }
    cout << max(maxi, ans1);
    // cout << ans1;
