    ...
    if (n % 2 == 0)
    {
      ...
      for (int i = 1; i < n + 1; i++)
      {
        ...
        else
        {
          if (i == 0)
            continue;
          oddPrefix[i] = oddPrefix[i - 1] + (max(0LL, max(v[i - 2], v[i]) - v[i - 1] + 1));
        }
      }
      for (int i = 1; i < n + 1; i++)
      {
        ...
        else
        {
          if (i == 1)
            continue;
          evenPrefix[i] = evenPrefix[i - 1] + (max(0LL, max(v[i - 2], v[i]) - v[i - 1] + 1));
        }
      }
      // cout << oddPrefix << endl;
      // cout << evenPrefix << endl;
      int res = LONG_LONG_MAX;
      for (int i = 0; i < n; i++)
      {
        if (i == 0)
        {
          res = min(res, evenPrefix[n] - evenPrefix[i + 1]);
        }
        else if (i != n - 2)
        {
          res = min(res, evenPrefix[n] - evenPrefix[i + 1] + oddPrefix[i + 1]);
        }
        else
        {
          res = min(res, oddPrefix[i + 1]);
        }
      }
