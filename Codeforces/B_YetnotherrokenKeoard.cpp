...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    // reverse(all(s));
    // cout << s;
    priority_queue<int> pq_lower;
    priority_queue<int> pq_upper;
    for (int i = 0; i < s.size(); i++)
    {
      ...
      else if (s[i] == 'b' && !(pq_lower.empty()))
      {
        s[pq_lower.top()] = '*';
        pq_lower.pop();
      }
      else if (s[i] == 'B' && !(pq_upper.empty()))
      {
        s[pq_upper.top()] = '*';
        pq_upper.pop();
      }
    ...
    // // reverse(all(s));
    // cout << pq_upper.size() << endl;
    // // ;
    // for (int i = 0; i < s.size(); i++)
    // {
    //   if (s[i] == 'B' && !(pq_upper.empty()))
    //   {
    //     cout << pq_upper.top() << endl;
    //     // s[pq_upper.top()] = '-';
    //     pq_upper.pop();
    //   }
    //   else if (s[i] == 'b' && !(pq_lower.empty()))
