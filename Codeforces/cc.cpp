...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    priority_queue<pair<int, pair<int, int>>> q; // guard, x,y;

    // queue<pair<int, pair<int, int>>> q; // guard, x,y;
    ...
    while (!q.empty())
    {
      ...
      for (int a = 0; a < 4; a++)
      {
        if (isValid(dx[a] + x, dy[a] + y) && vis[dx[a] + x][dy[a] + y] == 0)
        {
          if (grid[dx[a] + x][dy[a] + y] == 1)
          {
            q.push({-1 * (guard + 1), {dx[a] + x, dy[a] + y}});
          }
          else
          {
            q.push({-1 * guard, {dx[a] + x, dy[a] + y}});

            vis[dx[a] + x][dy[a] + y] = 1;
            debug(dx[a] + x, dy[a] + y, guard);
            res = max(res, guard);
          }
        }
      }
