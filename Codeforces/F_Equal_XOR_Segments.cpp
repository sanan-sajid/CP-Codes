...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    while (q--)
    {
      ...
      // for type 2`
      // b(l-1)=b(t), t should be at as right as possible
      // we can save pos of prexor in mao then do bs search in it
      debug(l, r);
      if (l != 0 && f != 1)
      {
        auto it1 = lower_bound(all(mp[preXor[l - 1]]), r);
        if (it1 == mp[preXor[l - 1]].begin())
        {
          if (f)
            cout << "YES";
          else
        ...
        it1--;
        auto it2 = lower_bound(all(mp[preXor[r]]), l);
        if (*it1 > *it2)
        {
          // debug(l, r, *it1, *it2);
          f = 1;
      ...
      {
        auto it1 = lower_bound(all(mp[0]), r);
        // cout << mp[0] << endl;
        // debug(it1 == mp[0].begin());/
        if (it1 == mp[0].begin())
        {
          if (f)
            cout << "YES";
