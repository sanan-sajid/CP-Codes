...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            prefixSum1[i] = prefixSum1[i - 1] + v1[i - 1];
        }
        vector<int> prefixSum2(n + 1, 0);
        for (int i = 1; i <= n; i++)
        {
            prefixSum2[i] = prefixSum2[i - 1] + v2[i - 1];
        }
        int temp = -1;
        int maxi = 0;
        for (int i = 1; i <= n; i++)
        {
            int idk = prefixSum1[i] - prefixSum1[0];
            idk += prefixSum2[n] - prefixSum2[i - 1];
            cout << idk << endl;
            if (maxi < idk)
            {
                temp = i;
        ...
        // cout << temp;
        // cout << max(prefixSum1[n] - prefixSum1[temp - 1], prefixSum2[temp] - prefixSum2[0]);
        cout << endl;
    }
    return 0;
}
