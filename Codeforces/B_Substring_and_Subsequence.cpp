...
pair<int, string> minLengthSubstringAndSubsequence(const string &a, const string &b)
{
  int m = a.length();
  int n = b.length();
  int i = 0, j = 0;
  ...
  // Append the remaining characters of a or b
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    auto [length, result] = minLengthSubstringAndSubsequence(a, b);
    cout << "Minimum length: " << length << endl;
