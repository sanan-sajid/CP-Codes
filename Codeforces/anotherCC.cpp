...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    {
      auto it = lower_bound(all(backward), forward[i]);
      auto it1 = upper_bound(all(backward), forward[i] + 2 * T);
      res += (it1 - it);
    }
    for (int i = 0; i < backward.size(); i++)
    {
      auto it = upper_bound(all(forward), backward[i]);
      auto it1 = lower_bound(all(forward), backward[i] - 2 * T);
      // debug(v[i], it1, it, it - it1);
      res += (it - it1);
    }
    cout << res / 2;
