...
// vector<vector<bool>> vis(105, false);
bool vis[105];
void dfs(int &startingNode, int currNode, vector<vector<int>> &adjList, vector<set<int>> &res)
{
  res[currNode].insert(startingNode);
  // if (vis[currNode] == true)
  //   return;
  // vis[currNode] = 1;
  for (auto it : adjList[currNode])
  {
    if (res[it].count(startingNode) == 0)
      dfs(startingNode, it, adjList, res);
  }
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 1; i <= n; i++)
    {
      ...
      for (auto it : res[i])
        cout << it << " ";
      // int temp = unique(all(res[i])) - res[i].begin();/
