  ...
  while (t--)
  {
    ...
    while (q--)
    {
      ...
      while (l < r)
      {
        ...
        int sum = sg.query(left, mid).sum;
        int idk = (sum * (2 * u - sum + 1)) / 2;
        int sum1 = sg.query(left, mid - 1).sum;
        int idk1 = (sum1 * (2 * u - sum1 + 1)) / 2;
        int sum2 = sg.query(left, mid + 1).sum;
        int idk2 = (sum2 * (2 * u - sum2 + 1)) / 2;
        if (idk >= res)
        {
          res = max(res, idk);
        }
    ...
    // for (int i = 0; i < n; i++)
    // {
    //   int sum = sg.query(0, i).sum;
    //   int idk = (sum * (2 * 8 - sum + 1)) / 2;
    //   cout << i + 1 << " " << idk << endl;
    // }
    cout << '\n';
  }
