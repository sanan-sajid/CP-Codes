...
bool dfs(map<char, set<char>> &mp, char node, vector<bool> &vis, char parent)
{
  // cout << node << " " << vis[node - 'a'] << endl;
  if (vis[node - 'a'])
    return 1;
  vis[node - 'a'] = 1;
  bool ans = 0;
  ...
  {
    if (it != parent)
      ans = (ans || dfs(mp, it, vis, node));
  }
  return ans;
}
void f(map<char, set<char>> &mp, char node, char parent, vector<int> &idk)
{
  idk[node - 'a'] = 1;
  cout << node;
  for (auto it : mp[node])
