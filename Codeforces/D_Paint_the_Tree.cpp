...
void dfs(int curr, int par, vector<int> &parent, vector<int> &depth, vector<vector<int>> &adj)
{
  for (auto it : adj[curr])
  {
    ...
    parent[it] = curr;
    depth[it] = 1 + depth[curr];
    dfs(it, curr, parent, depth, adj);
  }
}

vector<int> getpath(int x, int y, vector<vector<int>> &adj, vector<int> &parent, vector<int> &depth)
{
  if (depth[y] > depth[x])
  {
    swap(x, y);
  ...
  {
    pathx.push_back(pathy[i]);
  }
  return pathx;
}
int dfs1(vector<vector<int>> &adj, int node, int parent)
{
  bool f = 0;
  int maxi = -1e9;
  int res = 0;
  ...
  return (res - maxi) + 2 * (temp)-1;
}
int bfs(vector<vector<int>> &adj, int b, int a, int n)
{
  queue<int> q;
  q.push(b);
  int res = 0;
...
void dfs2(int curr, int par, int &maxdepth, int &level, vector<vector<int>> &adj)
{
  ...
  for (auto it : adj[curr])
  {
    ...
    dfs2(it, curr, maxdepth, level, adj);
    level--;
  }
}
int32_t main()
{
  ...
  while (t--)
  {
    ...
    dfs(1, 0, parent, depth, adj);
    vector<int> path = getpath(a, b, adj, parent, depth);
    if ((path[0] != a))
      reverse(path.begin(), path.end());

    int mid = (path.size() - 1) / 2; //(sz+1)/2 -1
    debug(path, mid, path[mid]);
    // vector<int>d2(n+1,0);
    vector<int> parent2(n + 1, 0);
    vector<int> depth2(n + 1, 0);
    dfs(a, 0, parent2, depth2, adj);
    int m = 0;
    int level = 0;
    dfs2(path[mid], 0, m, level, adj);
    cout << 2 * (n - 1) - m + (path.size() - 1 - (path.size() - 1) / 2);
    cout << '\n';
  }
  return 0;
}
