...
int m = 1e9 + 7;
int f(int node, int parent, int colour, vector<vector<int>> &dp)
{
  ...
  for (auto it : adj[node])
  {
    ...
    else
    {
      ...
      {
        // node is black
        modmul(ans, f(it, node, 1, dp), m);
      }
      else
      {
        // node is white
        modmul(ans, (f(it, node, 1, dp) % m + f(it, node, 0, dp) % m) % m, m);
      }
    }
  }
  debug(ans, node + 1, colour);
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    cout << (f(0, -1, 0, dp) % m + f(0, -1, 1, dp) % m) % m;
    cout << '\n';
  }
  return 0;
}
