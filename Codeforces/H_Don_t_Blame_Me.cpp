...
int f(int i, int mask, int &res, vector<vector<int>> &dp)
{
  if (i == 64)
  {
    if (mask == res)
  ...
  if (dp[i][mask] != -1)
    return (dp[i][mask] % m);
  // not taking
  int a = f(i + 1, mask, res, dp) % m;
  // taking
  int b = 0;

  if (mp[i])
  {
    b = (((modPower(2, mp[i]) - 1)) * (f(i + 1, (mask & i), res, dp) % m)) % m;
  }
  return dp[i][mask] = (((a % m) + (b % m)) % m);
}
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 0; i < 64; i++)
    {
      if (__popcount(i) == k)
      {
        vector<vector<int>> dp(64, vector<int>(200, -1));

        res = (res % m + f(0, temp, i, dp) % m) % m;
      }
    }
    cout << res % m;
    cout << '\n';
