...
int f(int ind, int remK, vector<int> &v, vector<vector<int>> &dp)
{
  if (ind >= v.size())
    return 0;
  if (dp[ind][remK] != -1)
  ...
  int ans = 1e18;
  int val = 1e18;
  for (int moves = 0; moves <= remK; moves++)
  {

    if (moves + ind >= v.size())
      break;
    val = min(val, v[moves + ind]);
    int temp = val * (moves + 1);
    ans = min(ans, temp + f(1 + moves + ind, remK - moves, v, dp));
  }
  return dp[ind][remK] = ans;
}
int32_t main()
{
