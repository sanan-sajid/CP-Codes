...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 1; i <= n; i++)
      cin >> v[i];
    vector<vector<int>> pre(n, vector<int>(320));
    map<int, vector<int>> mp1;
    map<int, vector<int>> mp2;
    for (int d = 1; d <= 317; d++)
    {
      vector<int> prefixSum(n + 5);
      prefixSum[0] = 0;
      for (int i = 1; i <= n; i++)
      {
        prefixSum[i] = v[i] + prefixSum[max(0LL, i - d)];
      }
      vector<int> anotherPrefixSum(n + 5);
      for (int i = 1; i <= n; i++)
      {
        anotherPrefixSum[i] = (ceil2(i, d) * v[i]) + anotherPrefixSum[max(0LL, i - d)];
      }
      ...
      // cout << endl;
      // for (auto it : anotherPrefixSum)
      //   cout << it << " ";

      // cout << endl;
      mp1[d] = prefixSum;
      mp2[d] = anotherPrefixSum;
    }
    ...
    while (q--)
    {
      ...
      if (d < 317)
      {
        cout << (mp2[d][s + (k - 1) * d] - mp2[d][max(0LL, s - d)]) - (max(0LL, ceil2(s, d) - 1)) * (mp1[d][s + (k - 1) * d] - mp1[d][max(0LL, s - d)]);
      }
      else
      {
        int res = 0;
        int temp = 1;
        for (int i = s; temp <= k; i += d)
        {
          res += ((temp++) * v[i]);
        }
