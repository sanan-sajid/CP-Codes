...
int modInverse(int n, int p = n1) // using fermats little thm. [p needs to be prime which is mostly the case as mod value generally is 1e15+7]
{
...
int f(int i, int prev, int idk)
{
  ...
  if (dp[i][prev][idk] != -1)
    return dp[i][prev][idk];
  if (i == 0)
  { // explore both
    return min(f(i + 1, 1, idk) + (s[i] == '1' ? 0 : coin[i]), f(i + 1, 0, idk) + (s[i] == '0' ? 0 : coin[i]));
  }
  int a, b; // a->alterante
  if (prev == 1)
  {
    a = f(i + 1, 0, idk) + (s[i] == '0' ? 0 : coin[i]);
    if (idk == 0)
    {
      b = f(i + 1, 1, 1) + (s[i] == '1' ? 0 : coin[i]);
    }
    else
    {
      b = 1e15;
  ...
  {
    // pre is 0
    a = f(i + 1, 1, idk) + (s[i] == '1' ? 0 : coin[i]);
    if (idk == 0)
    {
      b = f(i + 1, 0, 1) + (s[i] == '0' ? 0 : coin[i]);
    }
    else
    {
      b = 1e15;
