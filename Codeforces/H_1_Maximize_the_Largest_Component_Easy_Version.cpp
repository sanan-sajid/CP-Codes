...
void dfs(int i, int j, vector<string> &v, DSU &ds, vector<vector<int>> &vis)
{
  // debug(i, j);
  vis[i][j] = 1;
  // node numeber = i+j*m
  for (int a = 0; a < 4; a++)
  {
    if (valid(dx[a] + i, dy[a] + j) && v[dx[a] + i][dy[a] + j] == '#' && vis[dx[a] + i][dy[a] + j] == 0)
    {
      ds.join(i * m + j, m * (dx[a] + i) + (dy[a] + j));
      dfs(dx[a] + i, dy[a] + j, v, ds, vis);
    }
  }
}
int32_t main()
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      for (int j = 0; j < m; j++)
      {
        if (i != 0 && v[i - 1][j] == '#')
        {
          ...
          s.insert(ds.getParent(m * (i - 1) + j));
        }
        if (i != n - 1 && v[i + 1][j] == '#')
          s.insert(ds.getParent((i + 1) * m + j));
        s.insert(ds.getParent(i * m + j));
        debug(i * m + j, i, j);
      }
      // debug(i, temp);
      // debug(s);
