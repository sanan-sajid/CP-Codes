...
bool dfs(int node, int parent, vector<set<int>> &adjList, vector<bool> &vis)
{
  vis[node] = true;
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      adjList[i].insert(v[i] - 1);
      adjList[v[i] - 1].insert(i);
    }
    vector<bool> vis(n, false);
    int closedLoop = 0, open = 0;
    for (int i = 0; i < n; i++)
    {
      if (vis[i] == false)
      {
    ...
    // cout << open << " " << closedLoop;
    cout << closedLoop + min(1ll, open) << " " << closedLoop + open;
