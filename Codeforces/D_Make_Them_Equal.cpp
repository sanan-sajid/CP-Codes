...
int rec(int i, vector<pair<int, int>> &idk, int k, vector<vector<int>> &dp)
{
  if (i == idk.size())
  {
    if (k < 0)
  ...
  if (dp[i][k] != -1)
    return dp[i][k];
  // not taking
  int a = rec(i + 1, idk, k, dp);
  int b = rec(i + 1, idk, k - idk[i].first, dp) + idk[i].second;
...
int32_t main()
{
  ...
  vector<int> v(N, 1e9);
  v[1] = 0;
  v[0] = 0; // no need but for checking
  ...
  while (t--)
  {
    ...
    debug(*max_element(all(v)));
    // debug(idk);
    k = min(15 * n, k);

    vector<vector<int>> dp(n, vector<int>(15 * n + 1, -1));
    cout << rec(0, idk, k, dp);
