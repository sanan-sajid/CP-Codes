...
int f(int index, int a, int b, int health, vector<int> &v)
{
    if (index == v.size())
        return 0;
    int ans = 0;
    if (health > v[index])
    {
        return 1 + f(index + 1, a, b, health + v[index] / 2, v);
    }
    if (a)
        ans = max(ans, f(index, a - 1, b, health * 2, v));
    if (b)
        ans = max(ans, f(index, a, b - 1, health * 3, v));
    if (a && b)
        ans = max(ans, f(index, a - 1, b - 1, health * 6, v));
    if (a == 2 && b == 1)
        ans = max(ans, f(index, a - 2, b - 1, health * 12, v));
    return ans;
}
int32_t main()
{
