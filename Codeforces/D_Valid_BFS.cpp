...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 1; i <= n; i++)
    {
      sort(all(adj[i]), [&rev](const int &a, const int &b)
           { return rev[a] < rev[b]; });
    }
    vector<int> trav;
    queue<int> q;
    ...
    while (!q.empty())
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        q.pop();
        path.push_back(node);
        // temp.push_back(adj[node].size() - 1);
    ...
    // {
    //   sort(path.begin() + j, path.begin() + j + temp[i]);

    //   sort(ans.begin() + j, ans.begin() + j + temp[i]);
