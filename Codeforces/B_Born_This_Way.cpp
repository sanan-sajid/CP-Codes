...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    {
      // x ->nums of flights canceled going a to b
      int time = a[x + 1] + ta;
      int ind = lower_bound(all(b), time) - b.begin();
      ind += (k - x - 1);
      if (ind >= m)
        noPos = 1;

      else
        res = max(res, tb + b[ind]);
    }
    // take seperate case when no flught is canceled from a to b wla
    int time = a[0] + ta;
    int ind = lower_bound(all(b), time) - b.begin();
    ind += (k);
    if (ind >= m)
