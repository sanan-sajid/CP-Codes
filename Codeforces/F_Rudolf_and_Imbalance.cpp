  ...
  while (t--)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < m; i++)
      {
        ...
        {
          int mid = (l + r) / 2;
          int it = lower_bound(all(f), mid - d[i]) - f.begin();
          int temp = LONG_LONG_MAX;
          for (int p = max(0LL, it - 1); p <= min(k - 1, it + 1); p++)
          {
            // if (d[i] == 5 && f[p] == 9)
            // {
            //   cout << num1 - d[i] - f[p] << " " << d[i] + f[p] - num2 << endl;
            // }
            temp = min(temp, max(num1 - d[i] - f[p], d[i] + f[p] - num2));
            // cout << temp << endl;
          ...
          if (temp == LONG_LONG_MAX || temp >= num1 - num2)
          {
            // not poss
            l = mid + 1;
          }
