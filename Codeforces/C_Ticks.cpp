...
bool f(vector<string> &v, int i, int j, int k, vector<vector<bool>> &pos)
{
  int m = v[0].size();
  int n = v.size();
  int a = i - 1, b = j + 1;
  ...
  if (k <= (min(temp1, temp2)))
  {
    pos[i][j] = 1;
    int idk = min(temp1, temp2);
    // cout << i << " " << j << " " << idk << endl;
    int m = v[0].size();
    int n = v.size();
    int a = i - 1, b = j + 1;
    int temp11 = 0, temp22 = 0;
    while (a >= 0 && b < m)
    {
      pos[a][b] = 1;
      if (v[a][b] != '*')
