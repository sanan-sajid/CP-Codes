...
int32_t main()
{
  ...
  for (int k = 1; k <= t; k++)
  {
    ...
    else
    {
      ...
      //     << temp1;
      int j = 0;
      // cout << temp1.size() - (lower_bound(all(temp1), *max_element(all(temp1))) - temp1.begin());
      // cout << (lower_bound(all(temp1), *max_element(all(temp1))) - temp1.begin());
      for (int i = 0; i < n; i++)
      {
        if (ch[i] == '.')
        {
