...
unordered_map<char, int> mp, cost;
int a, b, c;
int r;
bool f(int x)
{
  int temp = cost['B'] * max(0LL, mp['B'] * x - a) + cost['S'] * max(0LL, mp['S'] * x - b) + cost['C'] * max(0LL, mp['C'] * x - c);
  // debug(temp, x, cost['C'], mp['c'] * x, max(0LL, mp['c'] * x - c));
  return (temp <= r);
}
int32_t main()
{
