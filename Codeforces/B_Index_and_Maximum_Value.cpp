...
vector<vector<int>> floydWarshall(int n, vector<vector<pair<int, int>>> &adj)
{
  ...
  vector<vector<int>> dist(n + 1, vector<int>(n + 1, LONG_LONG_MAX));
...
void rec(int i, vector<int> &temp, vector<vector<int>> &order, vector<pair<int, int>> &res)
{
  if (i == temp.size())
  {
    lol.push_back(res);
  ...
  int u = order[temp[i]][0];
  int v = order[temp[i]][1];
  res.push_back({u, v});
  rec(i + 1, temp, order, res);
  res.pop_back();
  res.push_back({v, u});
  rec(i + 1, temp, order, res);
  res.pop_back();
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    vector<vector<int>> dis = floydWarshall(n, adj);
    // debug(dis);
    int q;
    cin >> q;
    while (q--)
    {
      ...
      do
      {
        ...
        rec(0, temp, order, res);
        // debug(temp);
        // cout << endl;
      } while (next_permutation(all(roads)));
      ...
      for (auto it : lol)
      {
        ...
        int temp = dis[1][it[0].first] + dis[it.back().second][n];
        ...
        for (int i = 0; i < k - 1; i++)
        {
          temp += dis[it[i].second][it[i + 1].first];
        }
