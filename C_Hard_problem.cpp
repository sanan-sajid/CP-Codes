...
int dp[100001][2];
int f(int i, int prevRev, vector<vector<string>> &s, vector<int> &c)
{
  if (i == s.size())
    return 0;
  ...
  if (s[i - 1][prevRev] <= s[i][0])
    a = f(i + 1, 0, s, c);
  if (s[i - 1][prevRev] <= s[i][1])
    b = f(i + 1, 1, s, c) + c[i];
  return dp[i][prevRev] = min(a, b);
}
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    int res = min(f(1, 0, v, c), c[0] + f(1, 1, v, c));
    if (res == 1e18)
      cout << -1;
    else
      cout << res;
