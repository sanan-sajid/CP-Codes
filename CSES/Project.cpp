...
vector<int> dp;
int f(int i, vector<int> &start, vector<int> &end, vector<int> &score)
{
  if (i >= start.size())
    return 0;
  if (dp[i] != -1)
    return dp[i];
  int notTaking = f(i + 1, start, end, score);
  int ind = upper_bound(all(start), end[i]) - start.begin();
  int taking = score[i] + f(ind, start, end, score);
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    sort(all(v));
    vector<int> start, end, score;
    for (int i = 0; i < n; i++)
    {
      start.push_back(v[i][0]);
      end.push_back(v[i][1]);
      score.push_back(v[i][2]);
    }
    cout << f(0, start, end, score);
