...
int rec(int i, int j, vector<int> &v)
{
  ...
  if (dp[i][j] != -1)
    return dp[i][j];
  if ((v.size() % 2 != 0) && ((j - i + 1) % 2 != 0))

  {
    return dp[i][j] = max(v[i] + rec(i + 1, j, v), v[j] + rec(i, j - 1, v));
  }
  // p2 turn
  else if (v.size() % 2 != 0 && (j - i + 1) % 2 == 0)
  {
    // p2 also will try to maximize his score
    return dp[i][j] = min(rec(i + 1, j, v), rec(i, j - 1, v));
    // indirectly he will try to minimize p1 score
  }
  else
  {
    if ((i + j) & 1 == ((v.size() - 1) & 1))
