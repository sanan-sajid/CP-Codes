...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    set<int> temp(all(v));
    int res = 0;
    vector<int> vv(all(temp));
    for (int i = 0; i < vv.size(); i++)
    {
      int idk = vv[i];
      // auto ind = temp.lower_bound(idk + n - 1);
      auto ind = upper_bound(all(vv), idk + n - 1);
      if (ind == vv.end())
      {
        res = max(res, ll(vv.size()) - i);
      }
      else
      {
        int aaa = (ind - vv.begin());
        // cout << aaa - i << " " << vv[i] << " " << v[aaa] << endl;
        res = max(res, aaa - i);
      }
    }
    cout << res;
