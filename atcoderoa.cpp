...
int dif;
void merge(vector<int> &v, int l, int mid, int r, long long &res)
{
  vector<int> temp(r - l + 1);
  long long cnt = 0;
  for (int i = l; i <= mid; i++)
  {
    int ind = lower_bound(v.begin() + mid + 1, v.begin() + r + 1, v[i] - dif) - v.begin();
    debug(v);
    debug(v[i] - dif, l, r, mid, i, r - ind + 1);
    if (ind != r + 1)
      res += r - ind + 1;
  }
  int i = l;
...
void f(vector<int> &v, int l, int r, long long &res)
{
  if (r <= l)
    return;
  int mid = l + (r - l) / 2;
...
int32_t main()
{
  ...
  cin.tie(NULL);
  //
  vector<int> a = {-4, -4, 4, -1, -2, 5};
  vector<int> depth = {-2, 2, -1, 4, 4, 3};
  cout << numberOfPairs(a, a, 1);
  SegmentTree<int> sg(depth);
  sg.query(l, r).mx;
