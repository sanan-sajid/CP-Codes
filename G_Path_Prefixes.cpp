...
// pbdsMS s;
vector<int> pre;
void _dfs(int node, int parent)
{
  int temp = upper_bound(all(pre), dis[node]) - pre.begin();
  temp--;
  res[node] = temp + 1;
  // if (temp == pre.size())
  // else
  //   res[node] = temp + 1;
  debug(node + 1, pre, dis[node]);
  for (auto it : adj[node])
  {
    if (it[0] != parent)
    {
...
main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 1; i < n; i++)
    {
      ...
      cin >> u >> wt1 >> wt2;
      u--;
      adj[i].push_back({u, wt1, wt2});
      adj[u].push_back({i, wt1, wt2});
    }
    dfs(0, -1);
    debug(dis);
    _dfs(0, -1);
