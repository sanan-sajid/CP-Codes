...
struct segtree
{
  ...
  void set(int i, int v, int x, int lx, int rx, int type)
  {
    debug(x, type);
    if (rx - lx == 1)
    {
    ...
    {
      // go left
      set(i, v, 2 * x + 1, lx, mid, !type);
    }
    else
    {
      // go right
      set(i, v, 2 * x + 2, mid, rx, !type);
    }
    // send type =1 on set if
    if (type == 1)
    {
      // xor
  ...
  void set(int i, int v, int type)
  {

    set(i, v, 0, 0, size, type);
  }
