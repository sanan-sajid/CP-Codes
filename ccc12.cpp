...
//  4 1 4 3 3 2 5 2
//  i , j
int f(int i, int j, vector<int> &v, int &sum, vector<vector<int>> &dp)
{
  if (j - i + 1 <= 1)
    return 0;
  if (dp[i][j] != -1)
  ...
  if (v[i] + v[j] == sum)
  {
    ans = max(ans, 1 + f(i + 1, j - 1, v, sum, dp));
  }
  if (v[i] + v[i + 1] == sum)
  {
    ans = max(ans, 1 + f(i + 2, j, v, sum, dp));
  }
  if (v[j] + v[j - 1] == sum)
  {
    ans = max(ans, 1 + f(i, j - 2, v, sum, dp));
  }
  return dp[i][j] = ans;
}
...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    cout << max({f(0, n - 1, v, sum1, dp1), f(0, n - 1, v, sum2, dp2), f(0, n - 1, v, sum3, dp3)});
