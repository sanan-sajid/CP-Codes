...
int findTreeDiameter(int n, const vector<vector<pair<int, int>>> &adj)
{
  // First BFS to find one endpoint of the longest path
  pair<int, int> t1 = bfs(1, n, adj); // Starting from node 1

  // Second BFS to find the actual longest path from the endpoint found in the first BFS
  pair<int, int> t2 = bfs(t1.first, n, adj);

  // The distance in t2 is the diameter of the tree
  return t2.second;
}
int32_t main()
{
  ...
  while (t--)
  {
    ...
    int diameter = findTreeDiameter(n + 1, adj);
    debug(diameter, res, 2 * res);
    cout << 2 * res - diameter;
    cout << '\n';
  }
  return 0;
