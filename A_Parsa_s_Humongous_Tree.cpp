...
int f(int node, int parent, int type, vector<vector<int>> &adj, vector<vector<int>> &v, vector<vector<int>> &dp)
{
  // t0->l ,t1->r
  ...
  for (auto &it : adj[node])
  {
    if (parent == it)
      continue;
    ans += max(abs(v[node][type] - v[it][0]) + f(it, node, 0, adj, v, dp), abs(v[node][type] - v[it][1]) + f(it, node, 1, adj, v, dp));
    // debug(node, type, abs(v[node][type] - v[it][0]), abs(v[node][type] - v[it][1]));
  }
  return dp[node][type] = ans;
}
int32_t main()
{
  ...
  while (t--)
  {
    ...
    vector<vector<int>> dp1(n + 5, vector<int>(2, -1));

    cout << max(f(1, 0, 0, adj, v, dp), f(1, 0, 1, adj, v, dp1));
    // cout << f(1, 0, 1, adj, v, dp1);
