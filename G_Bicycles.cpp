...
int32_t main()
{
  ...
  while (t--)
  {
    ...
    vector<int> slow(n);
    cin >> slow;
    vector<vector<int>> dis(n, vector<int>(1005, 1e18));
    vector<int> temp(n, 1e18);
    set<vector<int>> s; //{dis,node,mini}
    s.insert({0, 0, slow[0]});
    dis[0][slow[0]] = 0;
    temp[0] = slow[0];
    while (!s.empty())
    {
      ...
      {
        if (dis[it1[0]][mini] > d + mini * it1[1])
        {
          dis[it1[0]][mini] = d + mini * it1[1];
          s.insert({dis[it1[0]][mini], it1[0], min(mini, slow[it1[0]])});
        }
      }
    }
